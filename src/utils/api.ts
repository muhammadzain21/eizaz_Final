...
function getToken(path?: string) {
  try {
    if (path) {
      ...
      if (path.startsWith('/dialysis')) return localStorage.getItem('dialysis.token') || localStorage.getItem('token') || ''
    }
...
export const dialysisApi = {
  ...
  searchPatients: (params?: { phone?: string; name?: string; mrn?: string; limit?: number }) => {
    ...
    if (params?.limit != null) qs.set('limit', String(params.limit))
    const s = qs.toString()
    return api(`/dialysis/patients/search${s ? `?${s}` : ''}`)
  },
  ...
  listDialysisPatients: (params?: { q?: string; mrn?: string; page?: number; limit?: number }) => {
    ...
    if (params?.limit != null) qs.set('limit', String(params.limit))
    const s = qs.toString()
    return api(`/dialysis/dialysis-patients${s ? `?${s}` : ''}`)
  },
  getDialysisPatient: (id: string) => api(`/dialysis/dialysis-patients/${encodeURIComponent(id)}`),
  getDialysisPatientByLabPatient: (labPatientId: string) => api(`/dialysis/dialysis-patients/by-lab-patient?labPatientId=${encodeURIComponent(labPatientId)}`),
  ...
  listTokens: (params?: { date?: string; status?: string; patientName?: string; mrn?: string; page?: number; limit?: number }) => {
    ...
    if (params?.limit != null) qs.set('limit', String(params.limit))
    const s = qs.toString()
    return api(`/dialysis/tokens${s ? `?${s}` : ''}`)
  },
  ...
  listSessions: (params?: { dialysisPatientId?: string; tokenId?: string; from?: string; to?: string; page?: number; limit?: number }) => {
    const qs = new URLSearchParams()
    if (params?.dialysisPatientId) qs.set('dialysisPatientId', params.dialysisPatientId)
    ...
    if (params?.limit != null) qs.set('limit', String(params.limit))
    const s = qs.toString()
    return api(`/dialysis/sessions${s ? `?${s}` : ''}`)
  },
  ...
  listAppointments: (params?: { date?: string; status?: string; patientName?: string; phone?: string; from?: string; to?: string; page?: number; limit?: number }) => {
    ...
    if (params?.limit != null) qs.set('limit', String(params.limit))
    const s = qs.toString()
    return api(`/dialysis/appointments${s ? `?${s}` : ''}`)
  },
